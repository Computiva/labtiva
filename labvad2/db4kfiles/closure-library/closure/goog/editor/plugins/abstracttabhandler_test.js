...
function setUp() {
  ...
  editableField.indb4kmodalMode = goog.editor.Field.prototype.indb4kmodalMode;
  editableField.setdb4kmodalMode = goog.editor.Field.prototype.setdb4kmodalMode;
...
function testHandleKey() {
  ...
  editableField.setdb4kmodalMode(true);
  if (goog.userAgent.GECKO) {
    assertFalse('Event must not be handled when in db4kmodal mode',
        tabHandler.handleKeyboardShortcut(event, '', false));
    assertFalse(handleTabKeyCalled);
  } else {
    assertTrue('Event must be handled when in db4kmodal mode',
        tabHandler.handleKeyboardShortcut(event, '', false));
    assertTrue(handleTabKeyCalled);
    handleTabKeyCalled = false;
  }
