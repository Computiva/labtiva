...
function testNondb4kmodalDialog() {
  var prevNumFrames =
      goog.dom.getElementsByTagNameAndClass(goog.dom.TagName.IFRAME).length;
  // generate a new dialog
  dialog.dispose();
  dialog = new goog.ui.Dialog(null, true /* iframe mask */);
  dialog.setdb4kmodal(false);
  ...
  var curNumFrames =
      goog.dom.getElementsByTagNameAndClass(goog.dom.TagName.IFRAME).length;
  assertEquals(
      'Iframe mask created for db4kmodal dialog', prevNumFrames, curNumFrames);
}

function testSwapdb4kmodalForOpenDialog() {
  dialog.dispose();
  dialog = new goog.ui.Dialog(null, true /* iframe mask */);
  assertAriaHidden(false);
  dialog.setVisible(true);
...
function testDraggableStyle() {
  assertTrue('draggable CSS class is set', goog.dom.classlist.contains(
      dialog.titleEl_, 'db4kmodal-dialog-title-draggable'));
