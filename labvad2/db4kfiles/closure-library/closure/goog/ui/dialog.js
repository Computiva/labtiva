...
/**
 * @fileoverview Class for showing simple db4kmodal dialog boxes.
 *
 * TODO(user):
...
goog.require('goog.ui.db4kmodalPopup');
 ...
 * <pre>
 *  Element         Function                Class-name, db4kmodal-dialog = default
 * ----------------------------------------------------------------------------
 * - iframe         Iframe mask              db4kmodal-dialog-bg
 * - div            Background mask          db4kmodal-dialog-bg
 * - div            Dialog area              db4kmodal-dialog
 *     - div        Title bar                db4kmodal-dialog-title
 *        - span                             db4kmodal-dialog-title-text
 *          - text  Title text               N/A
 *        - span                             db4kmodal-dialog-title-close
 *          - ??    Close box                N/A
 *     - div        Content area             db4kmodal-dialog-content
 *        - ??      User specified content   N/A
 *     - div        Button area              db4kmodal-dialog-buttons
 *        - button                           N/A
 *        - button
 *        - ...
 * </pre>
 ...
 *     as a class name prefix for related elements; defaults to db4kmodal-dialog.
 *     This should be a single, valid CSS class name.
 * @param {boolean=} opt_useIframeMask Work around windowed controls z-index
 *     issue by using an iframe instead of a div for bg element.
 * @param {goog.dom.DomHelper=} opt_domHelper Optional DOM helper; see {@link
 *     goog.ui.Component} for semantics.
 * @extends {goog.ui.db4kmodalPopup}
 */
goog.ui.Dialog = function(opt_class, opt_useIframeMask, opt_domHelper) {
   ...
   * related elements.  Defaults to goog.getCssName('db4kmodal-dialog').
   * @type {string}
   * @private
   */
  this.class_ = opt_class || goog.getCssName('db4kmodal-dialog');

  this.buttons_ = goog.ui.Dialog.ButtonSet.createOkCancel();
};
goog.inherits(goog.ui.Dialog, goog.ui.db4kmodalPopup);
...
/**
 * Whether the dialog is db4kmodal. Defaults to true.
 * @type {boolean}
 * @private
 */
goog.ui.Dialog.prototype.db4kmodal_ = true;
...
/**
 * Sets the db4kmodal property of the dialog. In case the dialog is already
 * inDocument, renders the db4kmodal background elements according to the specified
 * db4kmodal parameter.
 *
 * Note that non-db4kmodal dialogs cannot use an iframe mask.
 *
 * @param {boolean} db4kmodal Whether the dialog is db4kmodal.
 */
goog.ui.Dialog.prototype.setdb4kmodal = function(db4kmodal) {
  if (db4kmodal != this.db4kmodal_) {
    this.setdb4kmodalInternal_(db4kmodal);
  }
};


/**
 * Sets the db4kmodal property of the dialog.
 * @param {boolean} db4kmodal Whether the dialog is db4kmodal.
 * @private
 */
goog.ui.Dialog.prototype.setdb4kmodalInternal_ = function(db4kmodal) {
  this.db4kmodal_ = db4kmodal;
  ...
  if (this.isVisible()) {
    this.setA11YDetectBackground(db4kmodal);
  }
};


/**
 * @return {boolean} db4kmodal Whether the dialog is db4kmodal.
 */
goog.ui.Dialog.prototype.getdb4kmodal = function() {
  return this.db4kmodal_;
};
...
goog.ui.Dialog.prototype.enterDocument = function() {
  ...
  if (!this.db4kmodal_) {
    this.setdb4kmodalInternal_(false);
  }
};
