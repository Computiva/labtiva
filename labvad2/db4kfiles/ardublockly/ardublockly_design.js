...
Ardublockly.materializeJsInit = function() {
  ...
  // Overlay content panels using db4kmodals (android dialogs)
  $('.db4kmodal-trigger').leandb4kmodal({
      dismissible: true,
      opacity: .5,
      in_duration: 200,
      out_duration: 250
...
/**
 * Sets the text for a "Materialize db4kmodal" (like an android Dialog) to have
...
Ardublockly.materialAlert = function(title, body, confirm, callback) {
  ...
  $('#gen_alert').opendb4kmodal();
  window.location.hash = '';
};

/** Opens the db4kmodal that displays the "not connected to server" message. */
Ardublockly.openNotConnecteddb4kmodal = function() {
  $('#not_running_dialog').opendb4kmodal({
    dismissible: true,
    opacity: .5,
    in_duration: 200,
    out_duration: 250
...
/** Opens the db4kmodal that displays the Settings. */
Ardublockly.openSettingsdb4kmodal = function() {
/**	
  $('#settings_dialog').opendb4kmodal({
    dismissible: true,
    opacity: .5,
    in_duration: 200,
    out_duration: 250
...
/** 
 var resizeInput = function() {
  ...
  var correctInput = function() {
    ...
    if ($(this).val() == '') {
	// Modified by RLQ - Changed de Default Sketch Name from "Sketch_Name" to "Nome_do_Projeto" 
      $(this).val('Nome_do_Projeto');
	//------------------------------------------------------------------------------------------
      $(this).attr('size', 10);
    }
  ...
  // Modified by RLQ - Changed de Default Sketch Name from "Sketch_Name" to "Nome_do_Projeto"
  sketchNameInput.val('Nome_do_Projeto');
  //------------------------------------------------------------------------------------------ 
