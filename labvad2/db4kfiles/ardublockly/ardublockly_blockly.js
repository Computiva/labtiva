  ...
  request.onreadystatechange = function() {
    if ((request.readyState == 4) && (request.status == 200)) {
      Ardublockly.workspace = Blockly.inject(blocklyEl, {
            ...
            comments: true,
            disable: true,
            media: 'db4kfiles/blockly/media/',
            rtl: false,
            scrollbars: true,
            toolbox: request.responseText,
            trashcan: true });
      Ardublockly.BLOCKLY_INJECTED_ = true;
//Modificação DB4K - Insere na área de Blocos os blocos correpondentes ao programa carregado no editor;
 Ardublockly.replaceBlocksfromXml(localStorage.getItem("db4k_codigo"));
...
/**Modified by RLQ - The cod above was rewritten to the code bellow (only a translation to portuguese) **/
/** Discard all blocks from the workspace. */
Ardublockly.discardAllBlocks = function() {
  var blockCount = Ardublockly.workspace.getAllBlocks().length;
  if (blockCount == 1) {
    ...
    Ardublockly.materialAlert(
        ...
        'Existem ' + blockCount + ' blocos na área de trabalho. Você tem certeza ' +
        'que deseja apagá-los?',
        true,
        function() {
          Ardublockly.workspace.clear();
          Ardublockly.renderContent();
